...
; ALL_CAPS_IDENTIFIERS get the constant scope. Zed's tree-sitter engine
; doesn't support #lua-match? or #not-has-parent? from nvim-treesitter, so
; we use the standard #match? predicate. We lose the not-has-parent filter
; — in practice that just means CAPS identifiers everywhere get the
; constant color, which is usually what you want anyway.
((identifier) @constant
...
; Pascal-case identifiers default to types (Vec, Color, HashMap, ...). Must
; contain at least one lowercase letter so we don't fight with the ALL-CAPS
; constant rule above (DT_MAX, INITIAL_WIDTH stay constants).
((identifier) @type
  (#match? @type "^[A-Z][A-Za-z0-9_]*[a-z][A-Za-z0-9_]*$"))
